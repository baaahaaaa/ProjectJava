...
public class ListQuizController {
    ...
    private void showQuizDetails(Quiz quiz) {
        try {
            ...
            Parent root = loader.load(); // ← d'abord on charge !

            // ✅ Maintenant on peut accéder au contrôleur :
            QuizDetailsController controller = loader.getController();
            controller.setQuiz(quiz); // ← puis on appelle setQuiz
    ...
    private void showEditForm(Quiz quiz) {
        try {
            ...
            controller.setQuizToEdit(quiz); // ✅ ancienne méthode
            controller.setListQuizController(this); // ✅ nouvelle ligne
